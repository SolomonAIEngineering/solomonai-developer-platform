...
export const leadSourceEnum = pgEnum("leadSource", [
  "website",
  "referral",
  "social_media",
  "advertisement",
...
export const leads = pgTable("lead",{
    ...
    engagementLevel: integer("engagementLevel").notNull().default(0),

    // Communication Preferences
    preferredContactMethod: contactMethodEnum("preferredContactMethod"),
    optedIn: boolean("optedIn").default(false).notNull(),

    // Additional Data
    notes: text("notes"),

    // Cyclical Market Indicators
    seasonalInterest: jsonb("seasonalInterest").$type<{
      peakSeasons: string[]; // e.g., ["spring", "holiday"]
      offSeasons: string[];
      interestedProducts: string[];
    }>(),
    purchaseTimeline: text("purchaseTimeline"), // e.g., "immediate", "1-3 months", "next year"
    ...
    // Activity Tracking
    activityHistory: jsonb("activityHistory").$type<
      Array<{
        date: string;
        activityType: string;
        notes: string;
    ...
    ccpaConsent: boolean("ccpaConsent").default(false).notNull(),
    ...
    jsonb("insights").$type<
      Array<{
        ...
        recommendations: Array<{
          action: string;
          priority: typeof actionPriorityEnum;
          impact: {
            expected: number;
            timeframe: string;
            confidence: number;
