...
/**
 * EnrollMFA component for handling Multi-Factor Authentication enrollment.
 * This component provides a user interface for setting up MFA using a QR code or setup key.
 */
export function EnrollMFA() {
  ...
  /**
   * Handles the completion of OTP input.
   * Validates the entered code and redirects to the home page if successful.
  ...
  useEffect(() => {
    /**
     * Enrolls the user in MFA.
     * Sets up the necessary data for QR code and secret key.
     */
    async function enroll() {
      const { data, error } = await supabase.auth.mfa.enroll({
        factorType: "totp",
        issuer: "app-business.solomon-ai.app",
      });
      ...
      if (data.type === 'totp') {
        ...
        setSecret(data.totp.secret as any);
      }
    }
  ...
  /**
   * Handles the cancellation of MFA enrollment.
   * Unenrolls the user from MFA and redirects to the home page.
   */
