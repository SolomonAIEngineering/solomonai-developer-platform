...
async function Blog({ params, }) {
    ...
    return (<section id="blog">
      ...
      <div className="mx-auto w-full max-w-[800px] px-4 sm:px-6 lg:px-8 space-y-4 my-12">
        <react_1.Suspense fallback={<div className="mb-8 w-full h-64 bg-gray-200 animate-pulse rounded-lg"></div>}>
          {post.metadata.image && (<div className="mb-8">
              <image_1.default width={1920} height={1080} src={post.metadata.image} alt={post.metadata.title} className="w-full h-auto rounded-lg border shadow-md"/>
            </div>)}
        </react_1.Suspense>
        ...
        <div className="flex items-center space-x-2">
          <blog_author_1.default twitterUsername={post.metadata.author} name={post.metadata.author} image={"/author.jpg"}/>
        </div>
        <article className="prose dark:prose-invert mx-auto max-w-full" dangerouslySetInnerHTML={{ __html: post.source }}></article>
      </div>
      <cta_1.default />
    </section>);
}
