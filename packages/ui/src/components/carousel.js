...
const Carousel = React.forwardRef(({ orientation = "horizontal", opts, setApi, plugins, className, children, ...props }, ref) => {
    ...
    (0, react_hotkeys_hook_1.useHotkeys)("left", scrollPrev);
    (0, react_hotkeys_hook_1.useHotkeys)("right", scrollNext);
    React.useEffect(() => {
        if (!api || !setApi) {
            return;
        }
    ...
    return (<CarouselContext.Provider value={{
        ...
        <div ref={ref} className={(0, utils_1.cn)("relative", className)} role="region" aria-roledescription="carousel" {...props}>
          {children}
        </div>
...
const CarouselPrevious = React.forwardRef(({ className, variant = "outline", size = "icon", ...props }, ref) => {
    ...
    return (<button_1.Button ref={ref} variant={variant} size={size} className={(0, utils_1.cn)("absolute h-8 w-8 rounded-full", orientation === "horizontal"
      ...
      <icons_1.Icons.ChevronLeft className="h-6 w-6"/>
...
const CarouselNext = React.forwardRef(({ className, variant = "outline", size = "icon", ...props }, ref) => {
    ...
    return (<button_1.Button ref={ref} variant={variant} size={size} className={(0, utils_1.cn)("absolute h-8 w-8 rounded-full", orientation === "horizontal"
      ...
      <icons_1.Icons.ChevronRight className="h-6 w-6"/>
