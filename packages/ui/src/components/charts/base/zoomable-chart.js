...
exports.simulateData = simulateData;
exports.ZoomableChart = ZoomableChart;
 ...
 */
function simulateData(start = "2024-01-01T00:00:00Z", end = "2024-01-02T00:00:00Z") {
    const simulatedData = [];
    let baseValue = 50;
 ...
 */
function ZoomableChart({ data: initialData, description, title, dataNameKey = "events", height = 400, footerDescription, chartType = "area", }) {
    const [data, setData] = (0, react_1.useState)(initialData || []);
    const [refAreaLeft, setRefAreaLeft] = (0, react_1.useState)(null);
    const [refAreaRight, setRefAreaRight] = (0, react_1.useState)(null);
    const [startTime, setStartTime] = (0, react_1.useState)(null);
    const [endTime, setEndTime] = (0, react_1.useState)(null);
    const [originalData, setOriginalData] = (0, react_1.useState)(initialData || []);
     ...
     */
    const zoomedData = (0, react_1.useMemo)(() => {
        if (!startTime || !endTime) {
            return data;
        }
        const dataPointsInRange = originalData.filter((dataPoint) => dataPoint.date >= startTime && dataPoint.date <= endTime);
     ...
     */
    const total = (0, react_1.useMemo)(() => zoomedData.reduce((acc, curr) => acc + curr.events, 0), [zoomedData]);
    /**
    ...
    const handleZoom = (e) => {
        ...
        const currentRange = new Date(endTime || originalData[originalData.length - 1].date).getTime() - new Date(startTime || originalData[0].date).getTime();
    ...
    const renderChart = () => {
        if (chartType === "bar") {
            return (<recharts_1.Bar type="monotone" dataKey="events" stroke={chartConfig.events.color} fill={chartConfig.events.color} isAnimationActive={false}/>);
        }
        else if (chartType === "area") {
            return (<recharts_1.Area type="monotone" dataKey="events" stroke={chartConfig.events.color} fillOpacity={1} fill="url(#colorEvents)" isAnimationActive={false}/>);
        }
        else {
            return (<recharts_1.Line type="monotone" dataKey="events" stroke={chartConfig.events.color} isAnimationActive={false}/>);
        }
    };
    return (<card_1.Card className="w-full h-full">
      <card_1.CardHeader className="flex-col items-stretch space-y-0 border-b p-0 sm:flex-row hidden sm:flex">
        <div className="flex flex-1 flex-col justify-center gap-1 px-6 py-5 sm:py-6">
          {title && <card_1.CardTitle>{title}</card_1.CardTitle>}
      ...
      <card_1.CardContent className="px-2 sm:p-6 h-full sm:h-[calc(100%-150px)]">
        <chart_1.ChartContainer config={chartConfig} className="w-full h-full">
          <div className="h-full" onWheel={handleZoom} onTouchMove={handleZoom} ref={chartRef} style={{ touchAction: "none" }}>
            <div className="flex justify-end my-2 sm:mb-4">
              <button_1.Button variant="outline" onClick={handleReset} disabled={!startTime && !endTime} className="text-xs sm:text-sm">
                Reset
              </button_1.Button>
            </div>
            <recharts_1.ResponsiveContainer width="100%" height={400}>
              <recharts_1.ComposedChart data={zoomedData} margin={{
            top: 10,
            right: 10,
            left: 0,
            bottom: 0,
                ...
                <recharts_1.XAxis dataKey="date" tickFormatter={formatXAxis} tickLine={false} axisLine={false} tickMargin={4} minTickGap={16} style={{ fontSize: "10px", userSelect: "none" }}/>
                <recharts_1.YAxis tickLine={false} axisLine={false} style={{ fontSize: "10px", userSelect: "none" }} width={30}/>
                <chart_1.ChartTooltip cursor={false} content={<chart_1.ChartTooltipContent className="w-[150px] sm:w-[200px] font-mono text-xs sm:text-sm" nameKey={dataNameKey} labelFormatter={(value) => new Date(value).toLocaleString()}/>}/>
                <chart_1.ChartLegend content={<chart_1.ChartLegendContent nameKey={dataNameKey} hideIcon={false} hidden={false}/>}/>
                {renderChart()}
                {refAreaLeft && refAreaRight && (<recharts_1.ReferenceArea x1={refAreaLeft} x2={refAreaRight} strokeOpacity={0.3} fill="hsl(var(--foreground))" fillOpacity={0.05}/>)}
              </recharts_1.ComposedChart>
            </recharts_1.ResponsiveContainer>
          </div>
        </chart_1.ChartContainer>
      </card_1.CardContent>
      {footerDescription && (<card_1.CardFooter className="text-sm text-muted-foreground p-[3%] border-t">
          {footerDescription}
        </card_1.CardFooter>)}
    </card_1.Card>);
}
