...
exports.BankAccountsOverviewSummary = void 0;
...
const bank_account_card_1 = require("../cards/bank-account-card");
 ...
 */
const BankAccountsOverviewSummary = ({ financialProfile, financialContext, className, transactions, demoMode = false, }) => {
    ...
    let bankAccounts = linkedInstitutions
        ? linkedInstitutions
            ...
            .flat()
        : [];
    const validAccounts = bankAccounts.filter((account) => account !== undefined);
        ...
        <div className="flex-grow overflow-hidden">
          <tabs_1.Tabs defaultValue={validAccounts[0]?.name} className="flex h-full">
            <tabs_1.TabsList className="p-[1%] flex-col items-start justify-start h-[40%] overflow-y-auto scrollbar-hide w-fit mr-4 bg-black text-white rounded-2xl">
              {validAccounts.map((account, idx) => (<tabs_1.TabsTrigger value={account.name} key={idx} className="text-xs font-bold text-white text-left mb-2 w-full rounded-2xl">
            ...
            <card_1.Card className="overflow-y-auto scrollbar-hide w-full">
              {validAccounts.map((account, idx) => (<tabs_1.TabsContent value={account.name} key={idx} className="px-4">
                  <bank_account_card_1.BankAccountCard bankAccount={account} className="border-none bg-background text-foreground shadow-none" financialProfile={financialProfile} enableDemoMode={demoMode}/>
...
exports.BankAccountsOverviewSummary = BankAccountsOverviewSummary;
