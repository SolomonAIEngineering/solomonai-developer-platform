...
export const AiWriterView = ({ editor, node, getPos, deleteNode, }) => {
    ...
    const createItemClickHandler = useCallback((tone) => {
        return () => {
      ...
      <Panel noShadow className='w-full'>
        <div className='flex flex-col p-1'>
          ...
          {previewText && (<>
              ...
              <div className='relative mb-4 ml-2.5 max-h-[14rem] overflow-y-auto border-l-4 border-neutral-100 bg-white px-4 text-base text-black dark:border-neutral-700 dark:bg-black dark:text-white' dangerouslySetInnerHTML={{ __html: previewText }}/>
            </>)}
          ...
          <Textarea id={textareaId} value={data.text} onChange={onTextAreaChange} placeholder={'Tell me what you want me to write about.'} required className='mb-2'/>
          <div className='flex flex-row items-center justify-between gap-1'>
                ...
                <Dropdown.Portal>
                  <Dropdown.Content side='bottom' align='start' asChild>
                    <Surface className='min-w-[12rem] p-2'>
                      ...
                      {tones.map((tone) => (<Dropdown.Item asChild key={tone.value}>
            ...
            <div className='flex w-auto justify-between gap-1'>
              {previewText && (<Button variant='ghost' className='text-red-500 hover:bg-red-500/10 hover:text-red-500' onClick={discard}>
                  <Icon name='Trash'/>
                  Discard
                </Button>)}
              {previewText && (<Button variant='ghost' onClick={insert} disabled={!previewText}>
                  <Icon name='Check'/>
                  Insert
                </Button>)}
