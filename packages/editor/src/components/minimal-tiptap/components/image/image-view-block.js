        ...
        <figure>
          <div className='relative w-full' style={{
            ...
            <div className='absolute h-full w-full'>
              <div className={cn('relative h-full max-h-full w-full max-w-full rounded transition-all')} style={{
            boxShadow: editor.state.selection.from === getPos()
                ? '0 0 0 1px hsl(var(--primary))'
                : 'none',
        }}>
                <div className='relative flex h-full max-h-full w-full max-w-full overflow-hidden'>
                  <img alt={node.attrs.alt} src={node.attrs.src} className='absolute left-2/4 top-2/4 m-0 h-full max-w-full -translate-x-2/4 -translate-y-2/4 transform object-contain'/>
                </div>
              </div>
            </div>
          </div>
