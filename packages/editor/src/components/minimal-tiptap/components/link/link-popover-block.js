...
const LinkPopoverBlock = ({ link, onClear, onEdit, }) => {
    ...
    return (<div className='flex h-10 overflow-hidden rounded bg-background p-2 shadow-lg'>
      <div className='inline-flex items-center gap-1'>
        <ToolbarButton tooltip='Edit link' onClick={onEdit}>
          Edit link
        </ToolbarButton>
        <Separator orientation='vertical'/>
        <ToolbarButton tooltip='Open link in a new tab' onClick={() => window.open(link.href, '_blank')}>
        ...
        <Separator orientation='vertical'/>
        <ToolbarButton tooltip={copyTitle} onClick={handleCopy} tooltipOptions={{
