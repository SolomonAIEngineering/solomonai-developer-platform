...
export const EditorInfo = memo(({ characters, collabState, users, words }) => {
    return (<div className='flex items-center'>
        ...
        {collabState === 'connected' && (<div className='flex flex-row items-center'>
            <div className='relative ml-3 flex flex-row items-center'>
              {users.map((user) => (<div key={user.clientId} className='-ml-3'>
                  <Tooltip title={user.name}>
                    <img className='h-8 w-8 rounded-full border border-white dark:border-black' src={`https://api.dicebear.com/7.x/notionists-neutral/svg?seed=${user.name}&backgroundColor=${user.color.replace('#', '')}`} alt='avatar'/>
                  </Tooltip>
                </div>))}
              {users.length > 3 && (<div className='-ml-3'>
