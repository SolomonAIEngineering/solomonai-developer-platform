...
export const ColorPicker = ({ color, onChange, onClear }) => {
    ...
    return (<div className='flex flex-col gap-2'>
      ...
      <input type='text' className='w-full rounded border border-neutral-200 bg-white p-2 text-black focus:outline-1 focus:outline-neutral-300 focus:ring-0 dark:border-neutral-800 dark:bg-black dark:text-white dark:focus:outline-neutral-700' placeholder='#000000' value={colorInputValue} onChange={handleColorUpdate} onBlur={handleColorChange}/>
      <div className='flex max-w-[15rem] flex-wrap items-center gap-1'>
        {themeColors.map((currentColor) => (<ColorButton active={currentColor === color} color={currentColor} key={currentColor} onColorChange={onChange}/>))}
