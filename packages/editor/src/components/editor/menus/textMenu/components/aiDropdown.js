...
export const AIDropdown = ({ onCompleteSentence, onEmojify, onFixSpelling, onMakeLonger, onMakeShorter, onSimplify, onTldr, onTone, onTranslate, }) => {
    const handleTone = useCallback((tone) => () => onTone(tone), [onTone]);
    const handleTranslate = useCallback((language) => () => onTranslate(language), [onTranslate]);
    return (<Dropdown.Root>
      <Dropdown.Trigger asChild>
        <Toolbar.Button className='text-purple-500 hover:text-purple-600 active:text-purple-600 dark:text-purple-400 dark:hover:text-purple-300 dark:active:text-purple-400' activeClassname='text-purple-600 hover:text-purple-600 dark:text-purple-400 dark:hover:text-purple-200'>
      ...
      <Dropdown.Content asChild>
        <Surface className='min-w-[10rem] p-2'>
          ...
          <Dropdown.Sub>
            ...
            <Dropdown.SubContent>
              <Surface className='flex max-h-[20rem] min-w-[15rem] flex-col overflow-auto p-2'>
                {tones.map((tone) => (<Dropdown.Item onClick={handleTone(tone.value)} key={tone.value}>
          ...
          <Dropdown.Sub>
            ...
            <Dropdown.SubContent>
              <Surface className='flex max-h-[20rem] min-w-[15rem] flex-col overflow-auto p-2'>
                {languages.map((lang) => (<Dropdown.Item onClick={handleTranslate(lang.value)} key={lang.value}>
