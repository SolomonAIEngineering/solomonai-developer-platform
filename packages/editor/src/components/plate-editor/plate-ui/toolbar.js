...
React.forwardRef(({ className, variant, size, isDropdown, children, pressed, ...props }, ref) => {
    return typeof pressed === 'boolean' ? (<ToolbarToggleGroup type='single' value='single' disabled={props.disabled}>
          <ToolbarToggleItem ref={ref} className={cn(toolbarButtonVariants({
            variant,
            size,
        }), isDropdown && 'my-1 justify-between pr-1', className)} value={pressed ? 'single' : ''} {...props}>
            {isDropdown ? (<>
        ...
        </ToolbarToggleGroup>) : (<ToolbarPrimitive.Button ref={ref} className={cn(toolbarButtonVariants({
            variant,
            size,
        }), isDropdown && 'pr-1', className)} {...props}>
          {children}
...
export const ToolbarGroup = withRef(({ className, children, noSeparator }, ref) => {
    const childArr = React.Children.map(children, (c) => c);
    if (!childArr || childArr.length === 0)
        return null;
    return (<div ref={ref} className={cn('flex', className)}>
      {!noSeparator && (<div className='h-full py-1'>
