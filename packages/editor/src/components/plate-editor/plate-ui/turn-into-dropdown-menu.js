...
export function TurnIntoDropdownMenu(props) {
    const value = useEditorSelector((editor) => {
        let initialNodeType = ELEMENT_PARAGRAPH;
        ...
        if (nodes.length > 0) {
            ...
            allNodesMatchInitialNodeType = nodes.every(([node]) => {
                const type = node?.type || ELEMENT_PARAGRAPH;
    ...
    return (<DropdownMenu modal={false} {...openState} {...props}>
      ...
      <DropdownMenuContent align='start' className='min-w-0'>
        ...
        <DropdownMenuRadioGroup className='flex flex-col gap-0.5' value={value} onValueChange={(type) => {
            // if (type === 'ul' || type === 'ol') {
          ...
          {items.map(({ value: itemValue, label, icon: Icon }) => (<DropdownMenuRadioItem key={itemValue} value={itemValue} className='min-w-[180px]'>
