...
import { mergeTableCells, TableProvider, unmergeTableCells, useTableBordersDropdownMenuContentState, useTableElement, useTableElementState, useTableMergeState, } from '@udecode/plate-table';
...
export const TableBordersDropdownMenuContent = withRef((props, ref) => {
    ...
    return (<DropdownMenuContent ref={ref} className={cn('min-w-[220px]')} side='right' align='start' sideOffset={0} {...props}>
...
export const TableFloatingToolbar = withRef(({ children, ...props }, ref) => {
    ...
    const mergeContent = canMerge && (<Button contentEditable={false} variant='ghost' isMenu onClick={() => mergeTableCells(editor)}>
        <Icons.combine className='mr-2 size-4'/>
        Merge
      </Button>);
    const unmergeButton = canUnmerge && (<Button contentEditable={false} variant='ghost' isMenu onClick={() => unmergeTableCells(editor)}>
    ...
    return (<Popover open={open} modal={false}>
        <PopoverAnchor asChild>{children}</PopoverAnchor>
        {(canMerge || canUnmerge || collapsed) && (<PopoverContent ref={ref} className={cn(popoverVariants(), 'flex w-[220px] flex-col gap-1 p-1')} onOpenAutoFocus={(e) => e.preventDefault()} {...props}>
            {unmergeButton}
            {mergeContent}
            {bordersContent}
          </PopoverContent>)}
...
export const TableElement = withHOC(TableProvider, withRef(({ className, children, ...props }, ref) => {
    const { colSizes, isSelectingCell, minColumnWidth, marginLeft } = useTableElementState();
    const { props: tableProps, colGroupProps } = useTableElement();
    return (<TableFloatingToolbar>
        <div style={{ paddingLeft: marginLeft }}>
          <PlateElement ref={ref} asChild className={cn('my-4 ml-px mr-0 table h-px w-full table-fixed border-collapse', isSelectingCell && '[&_*::selection]:bg-none', className)} {...tableProps} {...props}>
            <table>
              <colgroup {...colGroupProps}>
                {colSizes.map((width, index) => (<col key={index} style={{
