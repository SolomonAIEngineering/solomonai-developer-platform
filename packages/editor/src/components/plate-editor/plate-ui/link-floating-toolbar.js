...
export function LinkFloatingToolbar({ state }) {
    ...
    if (hidden)
        return null;
    const input = (<div className='flex w-[330px] flex-col'>
    ...
    const editContent = editState.isEditing ? (input) : (<div className='box-content flex h-9 items-center gap-1'>
      ...
      <LinkOpenButton className={buttonVariants({
            variant: 'ghost',
            size: 'sms',
        })}>
