...
export const TableCellElement = withRef(({ children, className, style, hideBorder, isHeader, ...props }, ref) => {
    const { element } = props;
    ...
    return (<PlateElement ref={ref} asChild className={cn('relative h-full overflow-visible border-none bg-background p-0', hideBorder && 'before:border-none', element.background ? 'bg-[--cellBackground]' : 'bg-background', !hideBorder &&
            cn(isHeader && 'text-left [&_>_*]:m-0', 'before:size-full', selected && 'before:z-10 before:bg-muted', "before:absolute before:box-border before:select-none before:content-['']", borders &&
                cn(borders.bottom?.size &&
                    `before:border-b before:border-b-border`, borders.right?.size && `before:border-r before:border-r-border`, borders.left?.size && `before:border-l before:border-l-border`, borders.top?.size && `before:border-t before:border-t-border`)), className)} {...cellProps} {...props} style={{
            '--cellBackground': element.background,
            ...style,
        }}>
      <Cell>
        <div className='relative z-20 box-border h-full px-3 py-2' style={{
            minHeight: rowSize,
        }}>
          {children}
        </div>

        {!isSelectingCell && (<div className='group absolute top-0 size-full select-none' contentEditable={false} suppressContentEditableWarning={true}>
            {!readOnly && (<>
