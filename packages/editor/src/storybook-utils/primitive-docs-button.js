...
export function RadixPrimitiveDocsButton({ name }) {
    return (<div className='dark !mb-6 flex gap-4'>
      ...
      <button>
        <a href={`https://www.radix-ui.com/docs/primitives/components/${name}#api-reference`} className='!text-gray-11 !text-sm'>
          <ExternalLinkIcon className='mr-2 h-3 w-3'/>
          API Reference
        </a>
      </button>
...
export function PrimitiveDocsButton({ docsUrl, apiReferenceUrl, }) {
    return (<div className='dark !mb-6 flex gap-4'>
      <button>
