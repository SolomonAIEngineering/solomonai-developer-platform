...
const baseAppUrl = process.env.VERCEL_ENV === 'production'
    ? 'https://business.solomon-ai.app'
    : 'http://localhost:3001';
const TransactionsEmail = ({ fullName = 'Solomon AI Customer', transactions = defaultTransactions, locale = 'en', }) => {
    ...
    return (<components_1.Html>
      <components_1.Tailwind>
        ...
        <components_1.Body className='mx-auto my-auto bg-[#fff] font-sans'>
          <components_1.Container className='mx-auto my-[40px] max-w-[600px] border-transparent p-[20px] md:border-[#E8E7E1]' style={{ borderStyle: 'solid', borderWidth: 1 }}>
            ...
            <table style={{ width: '100% !important', minWidth: '100%' }} className='w-full border-collapse'>
              ...
              <tbody style={{ width: '100%', minWidth: '100% !important' }}>
                {transactions?.map((transaction) => (<tr key={transaction.id} className='h-[45px] border-0 border-b-[1px] border-solid border-[#E8E7E1]'>
                    <td align='left'>
                ...
                '!text-[#00C969]')}>
                        <div className='flex items-center space-x-2'>
                          ...
                          {transaction.status === 'pending' && (<div className='flex h-[22px] items-center space-x-1 rounded-md border px-2 py-1 text-xs text-[#878787]'>
                              <span>Pending</span>
                            </div>)}
                        </div>
                      </components_1.Link>
                    ...
                    <td align='left'>
                      <components_1.Text className={(0, cn_1.cn)('m-0 mt-1 p-0 pb-1 text-[14px] text-[#121212]', transaction?.category === 'income' &&
                '!text-[#00C969]')}>
            ...
            <components_1.Section className='mb-[32px] mt-[32px] text-center'>
              <components_1.Button className='rounded-md border border-solid border-[#121212] bg-transparent px-6 py-3 text-center text-[14px] font-medium text-[#121212] text-primary no-underline' href={`${baseAppUrl}/transactions?filter=${JSON.stringify({
            date: {
