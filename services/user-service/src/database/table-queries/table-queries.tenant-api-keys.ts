...
export class TenantAPIKeyQueries {
   ...
   */
  async getTenantAPIKeys(options?: QueryOptions & {
    tenantId?: string;
    isActive?: boolean;
    searchTerm?: string;
    scopes?: TenantAPIScope[];
  ...
  */
  private analyzeKeyUsage(logs: any[], timeframe: 'day' | 'week' | 'month') {
    ...
    const usageData = {
      ...
      byTime: this.analyzeTimeDistribution(logs, timeframe, intervals[timeframe]),
  ...
  private analyzeScopeRecommendations(key: any, logs: any[]) {
    ...
    logs.forEach(log => {
      if (log.metadata?.used_scopes) {
        log.metadata.used_scopes.forEach((scope: string) => usedScopes.add(scope));
      }
    });

    // Check for unused scopes
    const unusedScopes = key.scopes.filter((scope: string) => !usedScopes.has(scope));
  ...
  private analyzeUsageRecommendations(logs: any[]) {
    ...
    if (logs.length < 10) {
      recommendations.push({
        type: 'usage',
        priority: 'low',
        message: 'This API key has very low usage. Consider reviewing if it\'s still needed',
        action: 'review_usage'
      });
    }

    // Check for high error rates
    const errorRate = (logs.filter(log =>
      log.metadata?.status && log.metadata.status >= 400
  ...
  async generateUsageReport(
    ...
    const { startDate, endDate, includeInactive = false, format = 'summary' } = options || {};
  ...
  private async aggregateUsageMetrics(
    ...
    for (const key of keys) {
      ...
      metrics.error_count += logs.filter(log => {
        const status = log.metadata && typeof log.metadata === 'object' && 'status' in log.metadata ? log.metadata.status : undefined;
      ...
      const responseTimes = logs
        .map(log => {
          if (log.metadata && typeof log.metadata === 'object' && 'response_time' in log.metadata) {
      ...
      if (responseTimes.length > 0) {
        ...
        const newResponseTimeAvg = responseTimes.reduce((prev: number, curr: any) => {
          return prev + (typeof curr === 'number' ? curr : 0);
        }, 0) / responseTimes.length;
        metrics.average_response_time = metrics.average_response_time === 0
          ? newResponseTimeAvg
