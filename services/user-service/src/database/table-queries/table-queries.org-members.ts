...
export class OrgMemberQueries {
  ...
  private static readonly ROLE_PERMISSIONS: Record<MemberRole, OrgPermission[]> = {
    owner: [
      'manage_tenants',
      'view_usage',
      'manage_api_keys',
  ...
  async createMember(data: {
    ...
    await this.validateUniqueMember(data.organization_id, data.user_id, data.email);
  ...
  async bulkCreateMembers(
    ...
    const createdMembers = await this.prisma.$transaction(async (tx) => {
      ...
      for (const member of members) {
        ...
        await this.validateUniqueMember(organizationId, member.user_id, member.email);
   ...
   */
  async getMembers(options?: QueryOptions & {
    organizationId?: string;
    searchTerm?: string;
    roles?: MemberRole[];
    status?: MemberStatus[];
  ...
  async removePermissions(
    id: bigint,
    permissionsToRemove: OrgPermission[]
  ) {
    const member = await this.getMemberById(id);
    if (!member) throw new Error('Member not found');

    // Keep role-based permissions
    const rolePermissions = OrgMemberQueries.ROLE_PERMISSIONS[member.role as MemberRole];
    const updatedPermissions = member.permissions.filter(
      p => !permissionsToRemove.includes(p as OrgPermission) || rolePermissions.includes(p as OrgPermission)
    );
  ...
  async transferMember(
    ...
    await this.validateUniqueMember(newOrganizationId, member.user_id, member.email);
    ...
    if (!keepPermissions && keepRole) {
      updateData.permissions = OrgMemberQueries.ROLE_PERMISSIONS[member.role as MemberRole];
    }
  ...
  async getMemberActivitySummary(
    ...
    const activitySummary = auditLogs.reduce((summary: Record<string, number>, log) => {
  ...
  async getMemberPermissionsAudit(id: bigint) {
    ...
    const roleBasePermissions = OrgMemberQueries.ROLE_PERMISSIONS[member.role as MemberRole];
    ...
    return {
      ...
      hasCustomPermissions: additionalPermissions.length > 0 || missingRolePermissions.length > 0
    };
  }
  ...
  async getMemberStatistics(organizationId: string) {
    ...
    return {
      ...
      activeInLast30Days: members.filter(member =>
        member.last_access &&
  ...
  private async validateUniqueMember(
    ...
    if (existing) {
      throw new Error('Member with this user ID or email already exists in the organization');
    }
  }
 ...
 */
  ...
  async assignPermissionGroup(
    ...
    const permissionGroups = (org?.metadata as { permission_groups?: Record<string, any> })?.permission_groups || {};
  ...
  async checkSuspiciousActivity(
    ...
    if (loginAttempts > maxLoginAttempts) {
      ...
      suspiciousActivity.reasons.push(`Excessive login attempts: ${loginAttempts}`);
    }

    if (permissionChanges > maxPermissionChanges) {
      suspiciousActivity.suspicious = true;
      suspiciousActivity.reasons.push(`Unusual permission changes: ${permissionChanges}`);
    }
  ...
  async getMemberRoleRecommendations(memberId: bigint) {
    ...
    recentActivity.forEach(log => {
      if (typeof log.metadata === 'object' && log.metadata && 'used_permissions' in log.metadata) {
    ...
    const currentRolePermissions = new Set(OrgMemberQueries.ROLE_PERMISSIONS[currentRole]);
    ...
    return {
      ...
      analysis: {
        permissionUtilization: (usedPermissions.size / currentRolePermissions.size) * 100,
