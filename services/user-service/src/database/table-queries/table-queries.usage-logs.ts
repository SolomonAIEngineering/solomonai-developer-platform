...
export class UsageLoggingQueries {
     ...
     */
    async bulkLogOrganizationUsage(events: Array<{
        organization_id: string;
        event_type: UsageEventType;
        quantity: bigint;
        unit: UsageUnit;
    ...
    async getOrganizationUsageMetrics(
        ...
        const { startDate, endDate, eventTypes, groupBy, includeDetails } = options || {};
    ...
    async getUsageTrends(
        ...
        const { timeframe = 'month', eventTypes, compareWithPrevious = false } = options || {};
        ...
        return {
            ...
            previousPeriod: compareWithPrevious ?
                this.analyzeTrends(previousPeriodLogs, timeframe) : undefined,
            comparison: compareWithPrevious ?
                this.comparePeriods(currentPeriodLogs, previousPeriodLogs) : undefined
        };
    }
    ...
    private async getUsageLogs(
        ...
        const model = entityType === 'organization' ?
            Prisma.ModelName.org_usage_logs :
            Prisma.ModelName.tenant_usage_logs;
 ...
 */
   ...
   */
  private groupByTime(logs: any[], interval: 'hour' | 'day' | 'week' | 'month') {
    ...
    logs.forEach(log => {
      ...
      group.byEventType[log.event_type].byUnit[log.unit] += BigInt(log.quantity);
      ...
      group.byUnit[log.unit].byEventType[log.event_type] += BigInt(log.quantity);
    });
   ...
   */
  private getTimeKey(date: Date, interval: 'hour' | 'day' | 'week' | 'month'): string {
   ...
   */
  private analyzeTrends(logs: any[], timeframe: 'day' | 'week' | 'month' | 'year') {
    const groupedData = this.groupByTime(logs, timeframe === 'year' ? 'month' : 'day');
  ...
  private detectPeaks(values: number[]) {
    ...
    return {
      ...
      threshold,
      peaks: values.map((v, i) => v > threshold ? i : -1).filter(i => i !== -1)
    };
  }
