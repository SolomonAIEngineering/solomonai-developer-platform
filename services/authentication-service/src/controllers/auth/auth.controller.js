...
const CustomError_class_1 = __importStar(
  require("../../error/CustomError.class"),
);
const general_helper_1 = require("../../helpers/general.helper");
const cookie_1 = require("hono/cookie");
class Auth {
  async login(c) {
    const body = await (0, bodyParser_helper_1.parseBodyByContentType)(
      c,
      auth_schema_1.loginReqBodySchema,
    );
    ...
    if (loginError !== null) {
      if (loginError.message.includes("credentials")) {
        throw new CustomError_class_1.default(
          "AUTH-002",
          loginError,
          CustomError_class_1.ErrorTypes.AuthenticationError,
        );
      ...
      throw new CustomError_class_1.default(
        "Supabase",
        loginError,
        CustomError_class_1.ErrorTypes.SupabaseError,
      );
    }
  ...
  async register(c) {
    const body = await (0, bodyParser_helper_1.parseBodyByContentType)(
      c,
      auth_schema_1.resgisterReqBodySchema,
    );
  ...
  async forgotPassword(c) {
    const body = await (0, bodyParser_helper_1.parseBodyByContentType)(
      c,
      auth_schema_1.forgotPasswordReqBodySchema,
    );
    const supabaseClient = c.get("SERVICE_CLIENT");
  ...
  async resetPassword(c) {
    const body = await (0, bodyParser_helper_1.parseBodyByContentType)(
      c,
      auth_schema_1.resetPasswordReqBodySchema,
    );
    const supabaseClient = c.get("ANON_CLIENT");
  ...
  async changePassword(c) {
    const body = await (0, bodyParser_helper_1.parseBodyByContentType)(
      c,
      auth_schema_1.changePasswordReqBodySchema,
    );
    const session = c.get("CUSTOM_AUTH_SESSION");
    const supabaseClient = c.get("ANON_CLIENT");
  ...
  async initializeSession(c) {
    const body = await (0, bodyParser_helper_1.parseBodyByContentType)(
      c,
      auth_schema_1.initializeSessionReqBodySchema,
    );
    const supabaseClient = c.get("SERVICE_CLIENT");
