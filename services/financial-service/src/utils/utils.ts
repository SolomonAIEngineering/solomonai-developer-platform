...
class TestDataGenerator {
  ...
  private generateSeededRandom(): number {
    const hash = Array.from(this.seed + this.counter.toString()).reduce(
      (hash, char) => {
  ...
  private generateRandomString(length: number): string {
    ...
    for (let i = 0; i < length; i++) {
      const randomIndex = Math.floor(
        this.generateSeededRandom() * chars.length,
      );
  ...
  generateName(
    ...
    const randomFirst =
      firstNames[Math.floor(this.generateSeededRandom() * firstNames.length)];
    const randomLast =
      lastNames[Math.floor(this.generateSeededRandom() * lastNames.length)];
