...
import { registerV1ListPosts } from "./endpoints/posts/v1_list_posts.js";
import { registerV1ReadPost } from "./endpoints/posts/v1_read_posts.js";
import { registerV1UpsertPost } from "./endpoints/posts/v1_upsert_posts.js";
import { registerV1ReadQueue } from "./endpoints/queue/v1_read_queue.js";
import { registerV1WriteQueue } from "./endpoints/queue/v1_write_queue.js";
import { registerV1Liveness } from "./endpoints/v1_liveness.js";
import { createApp } from "./pkg/hono/index.js";
import { init } from "./pkg/middleware/init.js";
import { metrics } from "./pkg/middleware/metrics.js";
import { listPosts, readPost, upsertPost } from "./utils/index.js";
// =============================================================================
// Queries
// =============================================================================
// Routes
async function handleListRequest(env, origin) {
...
async function handleReadRequest(env, slug) {
...
async function handlePutRequest(request, env, slug) {
    if (slug === "/")
        return new Response("Method Not Allowed", { status: 405 });
    await upsertPost(env.DATABASE, slug, await request.text());
    return new Response(null, { status: 204 });
}
async function handleServiceDurableObjectRequest(request, env, slug) {
...
export async function handleR2Request(request, env, ctx) {
    if (request.method === "GET") {
...
export async function processJob(env, job) {
...
app.all("*", () => Response.json({
    success: false,
    error: "Route not found",
}, { status: 404 }));
// =============================================================================
// Entrypoint
const oldFetch = async (request, env, ctx) => {
    ...
    if (request.method === "GET") {
        ...
        if (pathname === "/service")
            ...
            return handleListRequest(env, origin);
        else
            return handleReadRequest(env, pathname);
    }
    else if (request.method === "PUT") {
...
export * from "./pkg/durable-object/index.js";
export { app };
