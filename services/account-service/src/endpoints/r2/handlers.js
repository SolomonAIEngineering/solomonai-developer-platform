...
export async function handleGetRequest(c) {
...
export async function handlePutRequest(c) {
    const key = c.req.param("key");
    const { R2_BUCKET } = c.env;
    try {
        ...
        return c.json({ message: "Record stored successfully", id: record.id }, 201);
    }
    catch (error) {
        if (error instanceof z.ZodError) {
            return c.json({ error: "Invalid record format", details: error.errors }, 400);
        }
        console.error(`Error putting object to R2: ${error}`);
        return c.json({ error: "Internal Server Error" }, 500);
    }
}
export async function handleDeleteRequest(c) {
    const key = c.req.param("key");
    const { R2_BUCKET } = c.env;
    try {
