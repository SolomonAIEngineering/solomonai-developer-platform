...
export class BusinessAccountDatabaseClient extends DatabaseClient {
    ...
    taxIdSchema = z.string().min(1).max(50);
    /**
     ...
     */
    async checkBusinessAccountExists(name) {
    ...
    async createBusinessAccount(params) {
        ...
        return this.executeTransaction(async (tx) => {
            const existingBusiness = await tx
                .select()
                .from(schema.businessAccounts)
                .where(or(eq(schema.businessAccounts.companyName, validatedParams.companyName)))
                .get();
            if (existingBusiness) {
                throw new DatabaseError({
     ...
     */
    async updateBusinessAccount(id, updateData) {
        try {
     ...
     */
    async softDeleteBusinessAccount({ id, }) {
        if (!id) {
            throw new DatabaseError({
                code: ErrorCode.BAD_REQUEST,
     ...
     */
    async hardDeleteBusinessAccount({ id }) {
        if (!id) {
            throw new DatabaseError({
                code: ErrorCode.BAD_REQUEST,
                message: "Business ID is required for deletion",
     ...
     */
    async searchBusinessAccounts(query, limit = 10, offset = 0) {
        const sanitizedQuery = query.replace(/[%_]/g, "\\$&"); // Escape special characters
        return this.executeQuery(async (db) => {
            return db
                .select()
                .from(schema.businessAccounts)
                .where(or(like(schema.businessAccounts.companyName, `%${sanitizedQuery}%`), like(schema.businessAccounts.email, `%${sanitizedQuery}%`)))
                .limit(limit)
                .offset(offset);
        });
    }
     ...
     */
    async getRecentlyCreatedBusinessAccounts(limit = 10) {
        return this.executeQuery(async (db) => {
            return db
                .select()
                .from(schema.businessAccounts)
     ...
     */
    async getBusinessWithAllRelations(businessId) {
        try {
