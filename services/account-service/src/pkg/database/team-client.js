...
export class TeamDatabaseClient extends DatabaseClient {
     ...
     */
    async searchTeams(query, limit = 10, offset = 0) {
        const sanitizedQuery = query.replace(/[%_]/g, "\\$&"); // Escape special characters
        return this.executeQuery(async (db) => {
            return db
                .select()
                .from(schema.teams)
                .where(or(like(schema.teams.name, `%${sanitizedQuery}%`), like(schema.teams.description, `%${sanitizedQuery}%`)))
                .limit(limit)
                .offset(offset);
        });
    }
     ...
     */
    async getRecentlyCreatedTeams(limit = 10) {
        return this.executeQuery(async (db) => {
            return db
                .select()
                .from(schema.teams)
     ...
     */
    async addTeamMember(teamId, memberId, memberType) {
        return this.executeTransaction(async (tx) => {
     ...
     */
    async removeTeamMember(teamId, memberId, memberType) {
        return this.executeQuery(async (db) => {
            const [result] = await db
                .delete(schema.teamMembers)
                .where(and(eq(schema.teamMembers.teamId, teamId), memberType === "user"
    ...
    async getTeamMembers(teamId) {
        return this.executeQuery(async (db) => {
            const userMembers = await db
                ...
                name: sql `COALESCE(${schema.userAccounts.username}, '')`.as("name"),
                type: sql `'user'`.as("type"),
            })
                .from(schema.teamMembers)
                .innerJoin(schema.userAccounts, eq(schema.teamMembers.userAccountId, schema.userAccounts.id))
                .where(eq(schema.teamMembers.teamId, teamId));
            const businessMembers = await db
                .select({
                id: schema.businessAccounts.id,
                name: sql `COALESCE(${schema.businessAccounts.companyName}, '')`.as("name"),
                type: sql `'business'`.as("type"),
            })
                .from(schema.teamMembers)
                .innerJoin(schema.businessAccounts, eq(schema.teamMembers.businessAccountId, schema.businessAccounts.id))
                .where(eq(schema.teamMembers.teamId, teamId));
            return [...userMembers, ...businessMembers];
        });
    }
    /**
     ...
     */
    async getTeamWithAllRelations(teamId) {
        try {
