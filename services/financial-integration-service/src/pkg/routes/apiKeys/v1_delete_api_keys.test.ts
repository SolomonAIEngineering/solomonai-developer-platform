...
import { IntegrationHarness } from "@/pkg/test-util/integration-harness";
import { TestDataGenerator } from "@/pkg/utils/utils";
...
describe("V1 Delete API Key Route", () => {
  ...
  beforeEach(async (task) => {
    ...
    const firstApiKeyResponse = await harness.post<
      V1CreateApiKeyRequest,
      V1CreateApiKeyResponse
    >({
    ...
    const secondApiKeyResponse = await harness.post<
      V1CreateApiKeyRequest,
      V1CreateApiKeyResponse
    >({
      url: Routes.ApiKeys.create.path,
  ...
  describe("DELETE /v1/api-keys - Cross-User Security", () => {
    test("should prevent deletion of other user's API keys", async () => {
      ...
      const secondUserResponse = await harness.post<
        V1CreateUserRequest,
        V1CreateUserResponse
      >({
        url: Routes.Users.create.path,
      ...
      const secondUserApiKey = await harness.post<
        V1CreateApiKeyRequest,
        V1CreateApiKeyResponse
      >({
        url: Routes.ApiKeys.create.path,
