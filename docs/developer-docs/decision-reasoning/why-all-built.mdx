...
```bash
bun turbo dev --filter=@orbitkit/web... --filter=!./packages/config/* --filter=!@v1/ui --only
```
...
{
  "scripts": {
    "dev:web:api": "bun turbo dev --filter=@orbitkit/web... --filter=!./packages/config/* --filter=!@v1/ui --only"
  }
}
```
