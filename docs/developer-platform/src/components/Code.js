...
function CodeGroupHeader({ title, children, selectedIndex }) {
  ...
  return (
    <div className="flex min-h-[calc(theme(spacing.12)+1px)] flex-wrap items-start gap-x-4 border-b border-zinc-700 bg-zinc-800 px-4 dark:border-zinc-800 dark:bg-transparent">
      ...
      {hasTabs && (
        <react_1.TabList className="-mb-px flex gap-4 text-xs font-medium">
          {react_2.Children.map(children, (child, childIndex) => (
            <react_1.Tab
              className={(0, clsx_1.default)(
            ...
            >
              {getPanelTitle(
                (0, react_2.isValidElement)(child) ? child.props : {},
              )}
            </react_1.Tab>
          ))}
        </react_1.TabList>
...
function CodeGroupPanels({ children, ...props }) {
  ...
  if (hasTabs) {
    return (
      <react_1.TabPanels>
        {react_2.Children.map(children, (child) => (
          <react_1.TabPanel>
...
function CodeGroup({ children, title, ...props }) {
  ...
  return (
    <CodeGroupContext.Provider value={true}>
      {hasTabs ? (
        <react_1.TabGroup {...tabGroupProps} className={containerClassName}>
